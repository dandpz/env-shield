#!/bin/sh
# env-shield installer: downloads the latest `evs` release binary for this
...
#   EVS_INSTALL_DIR  install directory (default: ~/.local/bin)
#   EVS_VERSION      version to install, e.g. "0.2.0" (default: latest release)
set -eu

REPO="dandpz/env-shield"
INSTALL_DIR="${EVS_INSTALL_DIR:-$HOME/.local/bin}"
...
name="env-shield-${version}-${target}"
base_url="https://github.com/$REPO/releases/download/v${version}"

tmpdir=$(mktemp -d)
trap 'rm -rf "$tmpdir"' EXIT

echo "Downloading env-shield v${version} (${target})..."
...
mkdir -p "$INSTALL_DIR"
install -m 755 "$tmpdir/$name/evs" "$INSTALL_DIR/evs"

echo "Installed evs v${version} to $INSTALL_DIR/evs"
case ":$PATH:" in
    *":$INSTALL_DIR:"*) ;;
    *)
        echo
